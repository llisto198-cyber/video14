/*
  ...
  2. Seguridad
    ...
    - Políticas que permiten acceso público de lectura y escritura
*/
...
-- Políticas de acceso público para novels
DROP POLICY IF EXISTS "Permitir acceso público a novels" ON novels;
...
DROP POLICY IF EXISTS "Permitir acceso público a delivery_zones" ON delivery_zones;
...
DROP POLICY IF EXISTS "Permitir acceso público a prices" ON prices;
...
DROP POLICY IF EXISTS "Permitir acceso público a system_config" ON system_config;
...
-- Políticas de acceso público para admin_credentials
DROP POLICY IF EXISTS "Permitir lectura pública de credenciales" ON admin_credentials;
